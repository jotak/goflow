...
func flowMessageFiltered(fmsg *flowmessage.FlowMessage) []flowMessageItem {
	...
	for _, field := range strings.Split(*MessageFields, ",") {
		...
		case "K8SSrcPodName":
			message = append(message, flowMessageItem{"K8SSrcPodName", fmt.Sprintf("%v", fmsg.K8SSrcPodName)})
		case "K8SSrcPodNamespace":
			message = append(message, flowMessageItem{"K8SSrcPodNamespace", fmt.Sprintf("%v", fmsg.K8SSrcPodNamespace)})
		case "K8SSrcPodNode":
			message = append(message, flowMessageItem{"K8SSrcPodNode", fmt.Sprintf("%v", fmsg.K8SSrcPodNode)})
		case "K8SDstPodName":
			message = append(message, flowMessageItem{"K8SDstPodName", fmt.Sprintf("%v", fmsg.K8SDstPodName)})
		case "K8SDstPodNamespace":
			message = append(message, flowMessageItem{"K8SDstPodNamespace", fmt.Sprintf("%v", fmsg.K8SDstPodNamespace)})
		case "K8SDstPodNode":
			message = append(message, flowMessageItem{"K8SDstPodNode", fmt.Sprintf("%v", fmsg.K8SDstPodNode)})
		}
	}
